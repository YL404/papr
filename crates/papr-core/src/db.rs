...
static MIGRATIONS: LazyLock<Migrations> = LazyLock::new(|| {
    Migrations::new(vec![M::up(
        ...
        // v20 — AI-summary provenance, shown under the summary in the reader
        // ("summarized by <model> · took 3.2s · 908 tokens"). All nullable:
        // summaries generated by older builds keep their text and simply show
        // no stats, and a provider that reports no token count leaves that
        // column NULL rather than storing a misleading zero.
        M::up(
            r#"
            ALTER TABLE articles ADD COLUMN ai_summary_model TEXT;
            ALTER TABLE articles ADD COLUMN ai_summary_ms INTEGER;
            ALTER TABLE articles ADD COLUMN ai_summary_tokens INTEGER;
            "#,
        ),
    ])
});
...
pub fn get_article(conn: &Connection, id: i64) -> AppResult<ArticleDetail> {
    let mut detail = conn.query_row(
        "SELECT a.id, a.feed_id, f.title, f.source_type, a.title, a.author, a.url,
                ...
                a.ai_summary_model, a.ai_summary_ms, a.ai_summary_tokens
        ...
        |r| {
            Ok(ArticleDetail {
                ...
                ai_summary_model: r.get(17)?,
                ai_summary_ms: r.get(18)?,
                ai_summary_tokens: r.get(19)?,
                enclosures: Vec::new(),
                tags: Vec::new(),
            })
        },
...
/// Persist a finished AI summary together with its provenance — the model that
/// produced it, how long it took, and how many tokens it cost — so the reader
/// can show them under the summary. `tokens` is `None` when the provider
/// reported no usage.
pub fn set_ai_summary(
    ...
    conn.execute(
        "UPDATE articles SET ai_summary = ?2, ai_summary_model = ?3,
                ai_summary_ms = ?4, ai_summary_tokens = ?5
         WHERE id = ?1",
        params![id, summary, model, elapsed_ms, tokens],
    )?;
    Ok(())
}
